...
def main(args):
    ...
    # Get all mask files in the prediction directory
    mask_files = [f for f in os.listdir(args.pred_dir) if f.endswith('_mask.npy')]

    print(f"Generating RLE for {len(mask_files)} images from {args.pred_dir}...")
    ...
    for file in tqdm(mask_files):
        ...
        labeled_mask = np.load(os.path.join(args.pred_dir, file))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--pred_dir", type=str, default="outputs/stage2_predictions", 
                        help="Directory containing the _mask.npy files")
